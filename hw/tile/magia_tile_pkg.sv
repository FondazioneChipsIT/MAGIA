...
package magia_tile_pkg;
  ...
  parameter int unsigned DWH     = 256+32;//544;                                        // Data Width for HWPE Interconnect: RedMulE Hx(P+1)xBits + Bank width = 8x(3+1)x16+32 
  ...
  parameter int unsigned REDMULE_DW         = DWH;                                                     // RedMulE Data Width: Hx(P+1)xBits + Bank width = 8x(1+1)x16+32 
  parameter int unsigned REDMULE_UW         = UWH;                                                     // RedMulE User Width
  parameter int unsigned REDMULE_ID_W       = magia_pkg::ID_W + 
                                              magia_pkg::ID_W_OFFSET;                                  // RedMulE ID Width                                                                                        // RedMulE HCI interface sizes
  parameter int unsigned                      REDMULE_AW            = magia_pkg::ADDR_W;               // RedMulE Address Width
  parameter int unsigned                      REDMULE_BW            = magia_pkg::BYTE_W;               // RedMulE Byte Width
...
`endif
  ...
  `HCI_TYPEDEF_REQ_T(redmule_data_req_t, logic[AWC-1:0], logic[DWH-1:0], logic[SWH-1:0], logic[UWH-1:0], logic[IW-1:0], logic)
  `HCI_TYPEDEF_RSP_T(redmule_data_rsp_t, logic[DWH-1:0], logic[UWH-1:0],  logic[IW-1:0], logic)
  ...
  `HCI_TYPEDEF_REQ_T(idma_hci_req_t, logic[iDMA_AddrWidth-1:0], logic[iDMA_DataWidth-1:0], logic[iDMA_StrbWidth-1:0], logic[iDMA_UserWidth-1:0], logic[IW-1:0], logic)
  `HCI_TYPEDEF_RSP_T(idma_hci_rsp_t, logic[iDMA_DataWidth-1:0], logic[iDMA_UserWidth-1:0], logic[IW-1:0], logic)
  ...
  parameter int unsigned MEM_DATA_W = magia_pkg::DATA_W;                                // Data Width memory (master ports)
  parameter int unsigned MEM_ADDR_W = 
                $clog2(magia_pkg::N_WORDS_BANK*magia_pkg::DATA_W/magia_pkg::BYTE_W);    // Address Width memory (master ports)
  parameter int unsigned MEM_BYTE_W = magia_pkg::BYTE_W;                                // Byte Width memory (master ports)
  parameter int unsigned MEM_USER_W = magia_pkg::USR_W;                                 // User Width memory (master ports)
  parameter int unsigned MEM_ID_W   = IW;                                               // Id Width memory (master ports)
parameter hci_package::hci_size_parameter_t HCI_SIZE_HWPE = '{
  ...
  parameter hci_package::hci_size_parameter_t HCI_SIZE_DMA = '{
    DW:  iDMA_DataWidth,
    AW:  iDMA_AddrWidth,
    BW:  magia_pkg::BYTE_W,
    UW:  iDMA_UserWidth,
